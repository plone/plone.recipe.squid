...
If generating these VHM-style URLs in a proxy *behind* Squid, no extra Squid
configuration is needed.  Just make sure the "backends" option sends the 
...
to recognize the path prefix of the VHM-style URL.
For example::
...
at 127.0.0.1 on port 8000.

Option 3 (rewrites within Squid):

To have Squid generate these VHM-style URLs, you can use the **zope_vhm_map** option. 
Here is an example::
...
Squid 2.6.STABLE18 or later.  But not Squid 3.0.
